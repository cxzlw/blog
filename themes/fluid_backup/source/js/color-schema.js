...
(function (window, document) {
    ...
    function toggleCustomColorSchema() {
        ...
        if (validColorSchemaKeys[currentSetting]) {
            ...
            if (!iconElement || !validColorSchemaKeys[currentSetting]) {
                ...
                currentSetting =
                    invertColorSchemaObj[getSchemaFromCSSMediaQuery()];
            }
        } else {
            return;
        }
    ...
    function setButtonIcon(schema) {
        if (validColorSchemaKeys[schema]) {
            ...
            if (
                document.documentElement.getAttribute("data-user-color-scheme")
            ) {
            ...
            document.querySelectorAll(".markdown-body pre").forEach((pre) => {
                var cls =
                    Fluid.utils.getBackgroundLightness(pre) >= 0
                        ? "code-widget-light"
                        : "code-widget-dark";
                var widget = pre.querySelector(
                    ".code-widget-light, .code-widget-dark",
                );
                if (widget) {
    ...
    function setApplications(schema) {
        ...
        if (giscus) {
            var giscusTheme =
                schema === "dark"
                    ? window.GiscusThemeDark
                    : window.GiscusThemeLight;
            const message = {
                setConfig: {
                    theme: giscusTheme,
                },
