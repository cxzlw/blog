...
module.exports = (hexo) => {
    ...
    function resolvePrism(name) {
        ...
        let file = resolveModule(
            "prismjs",
            `themes/${cssName ? "prism-" + cssName : "prism"}.css`,
        );
        if (!fs.existsSync(file)) {
    ...
    if (config.code.highlight.lib === "highlightjs") {
        ...
        hexo.theme.config.code.highlight.highlightjs = objUtil.merge(
            {},
            hexo.theme.config.code.highlight.highlightjs,
            {
                light: resolveHighlight(
                    hexo.theme.config.code.highlight.highlightjs.style,
                ),
                dark:
                    hexo.theme.config.dark_mode.enable &&
                    resolveHighlight(
                        hexo.theme.config.code.highlight.highlightjs.style_dark,
                    ),
            },
        );
        ...
        hexo.extend.filter.register("after_post_render", (page) => {
            if (hexo.config.highlight.line_number) {
                page.content = page.content.replace(
                    /<figure[^>]+?highlight.+?<td[^>]+?code[^>]+?>.*?(<pre.+?<\/pre>).+?<\/figure>/gims,
                    (str, p1) => {
                ...
                page.content = page.content.replace(
                    /(?<!<div class="code-wrapper">)<pre.+?<\/pre>/gims,
                    (str) => {
        ...
        hexo.theme.config.code.highlight.prismjs = objUtil.merge(
            {},
            hexo.theme.config.code.highlight.prismjs,
            {
                light: resolvePrism(
                    hexo.theme.config.code.highlight.prismjs.style,
                ),
                dark:
                    hexo.theme.config.dark_mode.enable &&
                    resolvePrism(
                        hexo.theme.config.code.highlight.prismjs.style_dark,
                    ),
            },
        );
            ...
            page.content = page.content.replace(
                ...
                (str) => {
                    if (/<code[^>]+?mermaid[^>]+?>/ims.test(str)) {
                        if (hexo.config.highlight.line_number) {
                            str = str.replace(
                                /<span[^>]+?line-numbers-rows[^>]+?>.+?<\/span><\/code>/,
                                "</code>",
                            );
                        }
                        str = str
                            .replace(/<pre[^>]*?>/gims, "<pre>")
                            .replace(
                                /(class=".*?)language-mermaid(.*?")/gims,
                                "$1mermaid$2",
                            )
